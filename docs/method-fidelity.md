...
| Cacheback | [paper](https://aclanthology.org/2025.emnlp-main.1581/) | Bounded LRU table of fixed-length leader/follower token n-grams, updated from emitted target tokens. Chain mode follows the most-recent follower; tree mode recursively branches recent followers. The optional frozen corpus-built cache is not yet loaded. | Shared greedy tree verification validates every cached path, so output is lossless. The missing frozen cache changes only cold-start coverage, not acceptance; it is an offline artifact rather than a claimed hot-path saving. |
